// QR Code generator (type 1-40) with Reed-Solomon error correction.
// Adapted from Kazuhiko Arase's qrcode-generator (MIT License).
// Only built-in Node.js modules are used.
...
const QRPolynomial = (num, shift) => {
  ...
  const get = (index) => _num[index];
  const getLength = () => _num.length;

  const multiply = (e) => {
    const numLength = getLength();
    const eLength = e.getLength();
    const newNum = new Array(numLength + eLength - 1).fill(0);
    for (let i = 0; i < numLength; i += 1) {
      for (let j = 0; j < eLength; j += 1) {
        newNum[i + j] ^= QRUtil.QRMath.gexp(QRUtil.QRMath.glog(get(i)) + QRUtil.QRMath.glog(e.get(j)));
      }
    }
    return QRPolynomial(newNum, 0);
  };

  const mod = (e) => {
    if (getLength() - e.getLength() < 0) return QRPolynomial(_num.slice(), 0);
    const ratio = QRUtil.QRMath.glog(get(0)) - QRUtil.QRMath.glog(e.get(0));
    const numLength = getLength();
    const newNum = _num.slice();

    for (let i = 0; i < e.getLength(); i += 1) {
      newNum[i] ^= QRUtil.QRMath.gexp(QRUtil.QRMath.glog(e.get(i)) + ratio);
    }

    return QRPolynomial(newNum, 0).mod(e);
  };

  return { get, getLength, multiply, mod };
};
...
const QR8bitByte = (data) => {
  const getMode = () => QRMode.MODE_8BIT_BYTE;
  const getLength = () => new TextEncoder().encode(data).length;
  const write = (buffer) => {
  ...
  return { getMode, getLength, write, data };
};

const QRCodeModel = (typeNumber, errorCorrectionLevel) => {
  const PAD0 = 0xec;
  const PAD1 = 0x11;
  let modules = null;
  let moduleCount = 0;
  ...
  const addData = (data) => {
    qrDataList.push(QR8bitByte(data));
  };
  ...
  const getBestMaskPattern = () => {
    ...
    for (let i = 0; i < 8; i += 1) {
      makeImpl(true, i);
      const lostPoint = QRUtil.getLostPoint(self);
  ...
  const makeImpl = (test, maskPattern) => {
    ...
    const data = createData(typeNumber, errorCorrectionLevel, qrDataList);
  ...
  const getErrorCorrectPolynomial = (errorCorrectionLength) => {
    let a = QRPolynomial([1], 0);
    for (let i = 0; i < errorCorrectionLength; i += 1) {
      a = a.multiply(QRPolynomial([1, QRUtil.QRMath.gexp(i)], 0));
    }
    return a;
  };

  const self = { addData, isDark, getModuleCount, make };
  Object.defineProperty(self, 'modules', {
    enumerable: true,
    get: () => modules,
  });
  return self;
...
function createQrModel(typeNumber, errorCorrectionLevel, data) {
  const qr = QRCodeModel(typeNumber, errorCorrectionLevel);
  qr.addData(data);
  qr.make();
  return qr;
}
